...
import styles from './AnswerLemon.module.css'
...
export const AnswerLemon = ({ answer, onCitationClicked, onExectResultClicked, sendFollowupQuestion }: Props) => {
  const { t } = useLanguage()
  ...
  const logoSrc = chatLogos[answer.organization ?? 'default'] || chatLogos.default
  const [isRefAccordionOpen, { toggle: toggleIsRefAccordionOpen }] = useBoolean(false)
  const filePathTruncationLimit = 50

  const isLoadingView = answer?.answer === t('chat.generatingAnswer')
