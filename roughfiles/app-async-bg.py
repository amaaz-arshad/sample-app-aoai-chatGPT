...
async def upload_files():
    ...
    # Read files synchronously (no await needed)
    files_data = []
    for uploaded_file in files:
        try:
            ...
            logging.error(f"Error reading {uploaded_file.filename}: {str(e)}")
    
    # Start background processing
    asyncio.create_task(process_files_background(files_data))
    
    return jsonify({
        "message": "File processing started in background",
        "status_check": "/pipeline/status"
    }), 202

async def process_files_background(files_data):
    processed_files = []
    skipped_files = []
    ...
    for file_data in files_data:
        ...
        try:
            # Process each file
            processed, skipped = await process_single_file(filename, organization, content)
            if processed:
...
async def process_single_file(file_name: str, organization: str, pdf_bytes: bytes):
    try:
        ...
        page_data = await loop.run_in_executor(
            executor,
            lambda: extract_pages_as_markdown(pdf_bytes, file_name)  # Pass file_name directly
        )
        ...
        for p in page_data:
            ...
            docs_array.append({
                ...
                "file": file_name,  # Use file_name directly
                "content": p["markdown"],
...
async def upload_status():
    status = getattr(current_app, "upload_status", {})
